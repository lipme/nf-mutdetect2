...
process SNPEFF_eff {
	...
	output:
	tuple val(sample), path("${sample}.snpeff.vcf.gz"),path("${sample}.snpeff.vcf.gz.tbi")

	script:
	snpeff_options = "-upDownStreamLen ${params.snpeff_up_downstream_len} -no-downstream  "

	"""
	snpEff eff ${snpeff_options}  -c ${snpeff_db}/snpeff.config refannot ${vcf} | bgzip -c > ${sample}.snpeff.vcf.gz
	tabix ${sample}.snpeff.vcf.gz
	"""
}
